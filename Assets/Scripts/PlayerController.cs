...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            Instantiate(flameSmitePref, enemy.position, Quaternion.identity);
            sb.TakeDamage(40);
        }
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
            ...
            sb.rb.AddForce((enemy.position - transform.position).normalized * jumpHeight, ForceMode2D.Impulse);
            RaycastHit2D hit = Physics2D.Raycast(attackRange.position, Vector2.right * transform.localScale.x, 2.9f, layerMask);
            if (hit.collider != null)
            {
                if (string.Equals(hit.collider.gameObject.name, "Slime"))
